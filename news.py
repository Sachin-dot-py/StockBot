...
class NewsTriggerDB():
    def __init__(self):
        ...
        self.conn.execute(
            """CREATE TABLE IF NOT EXISTS news_triggers (trigger TEXT, points REAL)"""
        )
    ...
    def addTrigger(self, trigger: str, points: int):
        ...
        if epoints:
            self.conn.execute(
                """UPDATE news_triggers SET points=? WHERE trigger=?""",
                (points, trigger))
        else:
            self.conn.execute("""INSERT INTO news_triggers values(?,?)""",
                              (trigger, points))
        self.conn.commit()

    def removeTrigger(self, trigger: str):
        self.conn.execute("""DELETE FROM news_triggers WHERE trigger=?""",
                          (trigger, ))
        self.conn.commit()

    def getTriggerPts(self, trigger: str) -> int:
        points = self.conn.execute(
            """SELECT * FROM news_triggers WHERE trigger=?""",
            (trigger, )).fetchone()
        return points

    def getAllTriggers(self) -> list:
        triggers = self.conn.execute(
            """SELECT * FROM news_triggers""").fetchall()
        return triggers
...
class NewsDB():
    ...
    def getAllNews(stock_id: str) -> list:
        ...
        for item in items:
            ...
            date_obj = datetime.strptime(
                ...
                "%a, %d %b %Y %H:%M:%S GMT") + timedelta(hours=8)
            ...
            if date_obj > datetime.now() - timedelta(hours=24):
    ...
    def removeDuplicates(self, stock_id: str, news: list) -> list:
        self.conn.execute(
            f"""CREATE TABLE IF NOT EXISTS [{stock_id}] (title TEXT, link TEXT, date TEXT, source TEXT, sourcelink TEXT)"""
        )
        news = [
            news_record for news_record in news if not self.conn.execute(
                f"""SELECT * FROM [{stock_id}] WHERE title=? AND source=?""",
                (news_record[0], news_record[3])).fetchone()
        ]
        return news
    ...
    def addNews(self, stock_id: str, news: list):
        ...
        self.conn.executemany(
            f"""INSERT INTO [{stock_id}] VALUES (?,?,?,?,?)""", news)
        self.conn.commit()
    ...
    def getImportant(self, stock_id: str, stock_name: str, news: list) -> list:
        ...
        for news_i in news:
            ...
            words = [
                self.lemmatizer.lemmatize(x) for x in tokenized_words
                if x not in self.stopwords and len(x) != 1
            ]
            for word in words:
